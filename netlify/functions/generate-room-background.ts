...
7. REALISTIC PROPORTIONS: since you are inventing the exact dimensions of anything you add (there is no specific real product being referenced here), stick to proportions that match what that type of item actually looks like in real life, AND relative to its surroundings:
   - Framed wall art is typically square to moderately landscape/portrait — not an unusually narrow, elongated sliver — unless the user specifically asked for a slim/narrow piece.
   - Table lamps placed on a dresser, console, or side table must be sized to that surface: base no wider than roughly a third of the surface's width, and total height (base + shade) should not exceed roughly half the height of an adult standing beside it. A table lamp must never be scaled to look like a floor lamp.
   - Floor lamps, plants, and other freestanding pieces should be judged against known real-world reference heights (a floor lamp is roughly 55-65 inches tall; a doorway in the photo is a reliable ~80 inch reference if visible).
   - For any added item, keep its proportions — and its size relative to nearby furniture, doorways, or people if present — within the normal real-world range for that object type and placement.
